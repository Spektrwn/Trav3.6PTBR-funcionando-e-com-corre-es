...
if(isset($_POST['result'])) {
	...
	echo "<p>Tipo de ataque: <b>";
	echo $form->getValue('ktyp') == 0? "Normal" : "Assalto";
	echo "</b></p>";
	if (isset($_POST['result'][3])&&isset($_POST['result'][4])){
		if ($_POST['result'][4]>$_POST['result'][3]){
			echo "<p>Edifício destruido</p>";
		}elseif ($_POST['result'][4]==0){
			echo "<p>O edifício não pode ser destruido</p>";
		}else{
			...
			echo "<p>Dano causado por catapulta: de nível <b>".$form->getValue('kata')."</b> para o nível <b>".$totallvl."</b></p>";
		}
	}
}
$target = isset($_POST['target'])? $_POST['target'] : array();
...
if(count($target) > 0) {
	...
	<thead>
		<tr>
			<th>
				Defensor <span class=\"small\"></span>
			</th>
		</tr>
	</thead>";
	foreach($target as $tar) {
...
<tbody><tr>
	<td>
		<label><input class="radio" type="radio" name="a1_v" value="1" <?php if($tribe == 1) { echo "checked"; } ?>> Romanos</label><br/>
		<label><input class="radio" type="radio" name="a1_v" value="2" <?php if($tribe == 2) { echo "checked"; } ?>> Teutões</label><br/>
		<label><input class="radio" type="radio" name="a1_v" value="3" <?php if($tribe == 3) { echo "checked"; } ?>> Gaulêses</label>
	</td><td>
		<label><input class="check" type="checkbox" name="a2_v1" value="1" <?php if(in_array(1,$target)) { echo "checked"; } ?>> Romanos</label><br/>

		<label><input class="check" type="checkbox" name="a2_v2" value="1" <?php if(in_array(2,$target)) { echo "checked"; } ?>> Teutões</label><br/>
		<label><input class="check" type="checkbox" name="a2_v3" value="1" <?php if(in_array(3,$target)) { echo "checked"; } ?>> Gaulêses</label><br/>
		<label><input class="check" type="checkbox" name="a2_v4" value="1" <?php if(in_array(4,$target)) { echo "checked"; } ?>> Natureza</label>
		</td><td>
		...
		<label><input class="radio" type="radio" name="ktyp" value="1" <?php if($form->getValue('ktyp') == 1) { echo "checked"; } ?>> assalto</label><br/>
