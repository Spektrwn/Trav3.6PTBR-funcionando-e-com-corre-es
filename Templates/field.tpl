...
<map name="rx" id="rx">
<?php 
for($i=1;$i<=18;$i++) {echo "	<area href=\"build.php?id=$i\" coords=\"$coorarray[$i]\" shape=\"circle\" title=\"".$building->procResType($arrayVillage['f'.$i.'t'])." Nível ".$arrayVillage['f'.$i]."\"/>\r\n";
}
?>
	<area href="dorf2.php" coords="144,131,36" shape="circle" title="Centro da aldeia" alt="" />
</map>
...
for($i=1;$i<=18;$i++) {
	if($arrayVillage['f'.$i.'t'] != 0) {
		$text = "";
		switch($i){
			case 1:$text = "Bosque Nível";break;
			case 2:$text = "Poço de Barro Nível";break;
			case 3:$text = "Mina de Ferro Nível";break;
			case 4:$text = "Campo de cereais Nível";break;
		}
