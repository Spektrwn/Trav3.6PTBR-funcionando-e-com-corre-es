    ...
    <table class="list" cellpadding="1" cellspacing="1">
        <thead>
            <tr>
                ...
                <td class="village sortable" onclick="Travian.Game.RaidList.sort(<?php echo $lid; ?>, 'village');">Aldeia</td>
                <td class="ew sortable" onclick="Travian.Game.RaidList.sort(<?php echo $lid; ?>, 'ew');">Ew</td>
                <td class="distance sortable" onclick="Travian.Game.RaidList.sort(<?php echo $lid; ?>, 'distance');">Distância</td>
                <td class="troops sortable" onclick="Travian.Game.RaidList.sort(<?php echo $lid; ?>, 'troops');">Tropas</td>
                <td class="lastRaid sortable" onclick="Travian.Game.RaidList.sort(<?php echo $lid; ?>, 'lastRaid');">Ultimo assalto</td>
                <td class="action"></td>
            </tr>
        </thead>
        <tbody>
...
if($query2 == 0) {        
    echo '<td class="noData" colspan="7">Não existe nenhuma lista de assalto.</td>';
}else{
...
<td class="checkbox">
        ...
        if (count($incoming_attacks) > 0) {
            ...
            if($inc_atts > 0) {
                echo '<img class="att2" src="img/x.gif" title="Enviando ataque" />';
            }
        }
        ?>
                ...
                <?php
                    ...
                    $oasistype = $type;/*Modificado por Spektrwn*/
                    if($oasistype != 0){
                ?>
...
$noticeClass = array("Relatório de Espião","Ganhou como atacante sem perdas","Ganhou como atacante com perdas","Perdeu como atacante com perdas","Ganhou como defensor sem perdas","Ganhou como defensor com perdas","Perdeu como defensor com Perdas","Perdido como defensor sem perdas","Reforço chegou","","Entregue em Madeira","Entregue em Barro","Entregue em Ferro","Entregue em Cereal","","Ganhou como defensor sem perdas","Ganhou como defensor com perdas","Perdeu como defensor com perdas","Espião ganhou como atacante","Espião perdeu como atacante","Espião ganhou como defensor","Espião perdeu como defensor");
...
?>
            ...
            <td class="action">
                <a class="arrow" href="build.php?id=39&t=99&action=showSlot&eid=<?php echo $id; ?>">editar</a>
            </td>
            </tr>
<?php
}
...
?>
    ...
    <div class="markAll">
		...
		<?php } ?>
        <label for="raidListMarkAll<?php echo $lid; ?>">Selecionar todos</label>
    </div>
...
$un = 1;
for($i=$start;$i<=$end;$i++){/* Voltar a mexer nesta parte = Spektrwn
 ...
 if($ct1=0 && i==1){ echo $ct1; }else{ echo 0; }
 if($ct2!=0 && i==2){ echo $ct2; }else{ echo 0; }
 if($ct3!=0 && i==3){ echo $ct3; }else{ echo 0; } 
 if($ct4!=0 && i==4){ echo $ct4; }else{ echo 0; } 
 if($ct5!=0 && i==5){ echo $ct5; }else{ echo 0; } 
 if($ct6!=0 && i==6){ echo $ct6; }else{ echo 0; } 
 if($ct7!=0 && i==7){ echo $ct7; }else{ echo 0; } 
 if($ct8!=0 && i==8){ echo $ct8; }else{ echo 0; } 
 if($ct9!=0 && i==9){ echo $ct9; }else{ echo 0; } 
 if($ct10!=0 && i==10){ echo $ct10; }else{ echo 0; }*/
}
?>
    <div class="addSlot">
<a href="build.php?id=39&t=99&action=addraid&lid=<?php echo $lid; ?>">Adicionar Assalto</a>
</div>
<div class="clear"></div>
...
<div class="round spacer listTitle" onclick="Travian.Game.RaidList.toggleList(<?php echo $lid; ?>);">
                        ...
                        <div class="openedClosedSwitch switchClosed">Detalhes</div>
                        <div class="clear"></div>
                    </div>
...
<?php } }?>
<div class="options">
    <a class="arrow" href="build.php?gid=16&t=99&action=addList">Criar uma nova lista</a>
</div>
<?php
}
