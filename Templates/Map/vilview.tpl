...
echo !$basearray['occupied']? $basearray['fieldtype']? "Vale Abandonado" : "Oasis não ocupado" : $basearray['name']; echo " (".$basearray['x']."|".$basearray['y'].")";
}else{
...
?>
<img src="img/x.gif" id="detailed_map" class="f99" alt="WW Plano de construção" />
<?php }else{ ?>
...
<table cellpadding="1" cellspacing="1" id="troop_info" class="tableNone">
        ...
        $unarray = array(31=>"Rato","Aranha","Cobra","Morcego","Javali","Lobo","Urso","Crocodilo","Tigre","Elefante");     
        $a = 0;
        for ($i = 31; $i <= 40; $i++) {
          if($unit['u'.$i]){
            echo '<tr>';
...
?>
	<table cellpadding="1" cellspacing="1" id="distribution" class="tableNone">
		...
		<thead><tr>
			<th colspan="3">Distribuição de terras</th>
		</tr></thead>
		<tbody>
						<tr>
				<td class="ico"><img class="r1" src="img/x.gif" alt="Madeira" title="Madeira" /></td>
				<td class="val"><?php echo $landd['0']; ?></td>
				<td class="desc">Bosques</td>

			</tr>
						<tr>
				<td class="ico"><img class="r2" src="img/x.gif" alt="Clay" title="Barro" /></td>
				<td class="val"><?php echo $landd['1']; ?></td>
				<td class="desc">Poços de Barro</td>
			</tr>
						<tr>
				<td class="ico"><img class="r3" src="img/x.gif" alt="Iron" title="Ferro" /></td>

				<td class="val"><?php echo $landd['2']; ?></td>
				<td class="desc">Minas de ferro</td>
			</tr>
						<tr>
				<td class="ico"><img class="r4" src="img/x.gif" alt="Crop" title="Cereal" /></td>
				<td class="val"><?php echo $landd['3']; ?></td>
				<td class="desc">Campos de cereais</td>
    ...
    <table cellpadding="1" cellspacing="1" id="village_info" class="tableNone">
		...
		<tbody><tr>
			<th>Tribo</th>
			<td><?php switch($uinfo['tribe']) { case 1: echo Romanos; break; case 2: echo Teutões; break; case 3: echo Gaulês; break; case 4: echo Natureza; break; case 5: echo Natars; break;} ?></td>
		</tr>
		<tr>
			<th>Aliança</th>
			<?php if($uinfo['alliance'] == 0){
...
<table cellpadding="1" cellspacing="1" id="options" class="tableNone">
	...
	<tbody><tr>
		...
		<td><a href="karte.php?z=<?php echo $_GET['d']; ?>">&raquo; Centralizar mapa.</a></td>
	</tr>
    ...
    ?>
      ...
      if($cps >= $need_cps) {
        ...
        $test = "<a href=\"a2b.php?id=".$_GET['d']."&amp;s=1\">&raquo;  Fundar nova aldeia.</a>";
      } elseif($village->unitarray['u'.$session->tribe.'0'] >= 3 AND !$enough_cp) {
        $test = "&raquo; Fundar nova aldeia. ($cps/$need_cps pontos de cultura)";
      } else {
        $test = "&raquo; Fundar nova aldeia. (".$village->unitarray['u'.$session->tribe.'0']."/3 colonos disponíveis)";
      }
		...
		"<a href=\"build.php?id=39\">&raquo; Assaltar oásis $otext. (Construir um ponto de reunião)</a>" : 
		"&raquo; Assaltar oásis $otext. (Construir um ponto de reunião)" : 
		
		
		"<a href=\"a2b.php?z=".$_GET['d']."&o\">&raquo; Assaltar oásis $otext .</a>" :
		"$test"
			?>
		</tr>
        <?php } 
        ...
        <tr>
          ...
          $data2 = mysql_fetch_assoc($query2);
			...
			echo "&raquo; Enviar tropas. (Jogador está banido)";
          } else if($data2['protect'] < time()) {
            echo $village->resarray['f39']? "<a href=\"a2b.php?z=".$_GET['d']."\">&raquo; Enviar tropas." : "&raquo; Enviar tropas. (Construir um ponto de reunião)"; 
          } else {
            echo "&raquo; Enviar tropas. (proteção de iniciante)";
          }
          ?>
          </td>
				</tr>
					...
					<?php
			if($data2['access']=='0' or $data2['access']=='8' or $data2['access']=='9') {
			echo "&raquo; Enviar mercador(es). (Banido)";
          } else {
            echo $building->getTypeLevel(17)? "<a href=\"build.php?z=".$_GET['d']."&id=" . $building->getTypeField(17) . "\">&raquo; Enviar mercador(es)." : "&raquo; Enviar mercador(es). (construir mercado)"; 
          }
