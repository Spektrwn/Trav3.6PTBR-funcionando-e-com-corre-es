...
<table id="distribution" cellpadding="1" cellspacing="1"> 
	<thead><tr>
		<th colspan="5">
			...
			?>
			Nível <?php echo $hero_info['level']; ?> <span class="info">( <?php echo"<img class=\"unit u".$hero_info['unit']."\" src=\"img/x.gif\" alt=\"".$technology->getUnitName($hero_info['unit'])."\" title=\"".$technology->getUnitName($hero_info['unit'])."\" /> ".$technology->getUnitName($hero_info['unit']); ?> )</span></th>
	</tr></thead> 
    <tbody><tr> 
        <th>Ataque</th> 
    ...
    <tr> 
        <th>Regeneração</th> 
        <td class="val"><?php echo ($hero_info['regeneration']*5*SPEED); ?>/Dia</td> 
	...
	<?php }else{ ?>
        <th title="until the next level">Experiência:</th> 
        <td class="val">100%</td> 
	...
	<?php if(isset($_GET['e'])){ 
        echo "<p><font size=\"1\" color=\"red\"><b>Error: Nome muito curto</b></font></p>"; 
    } 
    ?> 
    <?php if($hero_info['level'] <= 3){ ?> 
        <p>Você pode <a href="build.php?id=<?php echo $id; ?>&add=reset">Limpar</a> seus pontos até que você esteja nível <b>3</b> ou menos!</p> 
    <?php } ?> 
     
<p>Seu heroi possui <b><?php echo floor($hero_info['health']); ?></b>% dos pontos de vida.<br/>  
    Seu herois já conquistou <b><?php echo $database->VillageOasisCount($village->wid); ?></b> <a href="build.php?id=<?php echo $id; ?>&land">oasis</a>.</p> 
