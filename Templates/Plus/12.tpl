...
$done1 = "+25% Produção: Cereal";
    mysql_query("UPDATE ".TB_PREFIX."users set gold = ".($session->gold-7)." where `id`='".$session->uid."'") or die(mysql_error());
    mysql_query("INSERT INTO ".TB_PREFIX."gold_fin_log VALUES ('".(mysql_num_rows($goldlog)+1)."', '".$village->wid."', '+25%  Produção: Cereal')") or die(mysql_error());

} else {
$done1 = "nada foi feito";
    mysql_query("INSERT INTO ".TB_PREFIX."gold_fin_log VALUES ('".(mysql_num_rows($goldlog)+1)."', '".$village->wid."', 'Falhou +25%  Produção: Cereal')") or die(mysql_error());
