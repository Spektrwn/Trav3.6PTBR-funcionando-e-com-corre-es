...
<?php
if($building->walling()) {
	$wtitle = $building->procResType($building->walling())." Nível ".$village->resarray['f40'];
}
else {
	$wtitle = ($village->resarray['f40'] == 0)? "Local de construção exterior" : $building->procResType($village->resarray['f40t'],0)." Nível ".$village->resarray['f40'];
}
?>
...
echo "<area href=\"build.php?id=99\" title=\"Maravilha do Mundo Nível ".$village->resarray['f99']."\" coords=\"$coords[$t]\" shape=\"poly\"/>";
} else {
if($village->resarray['f'.$t.'t'] != 0) {
$title = $building->procResType($village->resarray['f'.$t.'t']). " Nível ".$village->resarray['f'.$t];
}
else {
    $title = "Espaço para construção";
...
for ($i=1;$i<=20;$i++) {
	...
	$text = "Espaço para construção";
	$img = "iso";
    	if($village->resarray['f'.($i+18).'t'] != 0) {
        	$text = $building->procResType($village->resarray['f'.($i+18).'t'])." Nível ".$village->resarray['f'.($i+18)];
            $img = "g".$village->resarray['f'.($i+18).'t'];
       }
    foreach($building->buildArray as $job) {
    	if($job['field'] == ($i+18)) {
        	$img = 'g'.$job['type'].'b';
            $text = $building->procResType($job['type'])." Nível ".$village->resarray['f'.$job['field']];
        }
    }
    ...
    if($village->resarray['f39'] == 0) {
            ...
            echo "<img src=\"img/x.gif\" class=\"dx1 g16b\" alt=\"Ponto de Reunião Nível ".$village->resarray['f39']."\" />";
            }
            else {
            echo "<img src=\"img/x.gif\" class=\"dx1 g16e\" alt=\"Local de construção do Ponto de Reunião\" />";
            }
        }
      else {
      	echo "<img src=\"img/x.gif\" class=\"dx1 g16\" alt=\"Ponto de Reunião Nível ".$village->resarray['f39']."\" />";
      }
?>
<?php
if($village->resarray['f99t'] == 40) {
if($village->resarray['f99'] >= 0 && $village->resarray['f99'] <= 19) {
echo '<img class="ww g40" src="img/x.gif" alt="Maravilha do Mundo">'; }
if($village->resarray['f99'] >= 20 && $village->resarray['f99'] <= 39) {
echo '<img class="ww g40_1" src="img/x.gif" alt="Maravilha do Mundo">'; }
if($village->resarray['f99'] >= 40 && $village->resarray['f99'] <= 59) {
echo '<img class="ww g40_2" src="img/x.gif" alt="Maravilha do Mundo">'; }
if($village->resarray['f99'] >= 60 && $village->resarray['f99'] <= 79) {
echo '<img class="ww g40_3" src="img/x.gif" alt="Maravilha do Mundo">'; }
if($village->resarray['f99'] >= 80 && $village->resarray['f99'] <= 99) {
echo '<img class="ww g40_4" src="img/x.gif" alt="Maravilha do Mundo">'; }
if($village->resarray['f99'] == 100) {
echo '<img class="ww g40_5" src="img/x.gif" alt="Maravilha do Mundo">'; }
}
?>
