...
## Authors:     aggenkeech - and a little help from Eyas95      ##
 ##         / Alterado/Corrigido e Traduzido por Spektrwn         ##
## Created:     31/03/2012                                      ##
##################################################################
...
$q = "
	...
	$sql = mysql_query("SELECT endtime FROM ".TB_PREFIX."movement order by moveid desc limit 1")or die(mysql_error());
	$wingsrtime = mysql_result($sql, 0);
...
d17/x3f8m6l++P5/85njqecf6NH/s8/0/wHeyCdEnMVhkQAAAABJRU5ErkJggg==" align="right" style="padding-top: 40px;">
					<p>
					<b>Caros jogadores do <?php echo SERVER_NAME;?>,</b>
					<br /><br />
					Todas as coisas boas devem chegar ao fim, e essa era também deve. Uma vez que o edifício recebeu uma coroa, sobre o qual estava inscrita uma mensagem que poderia levar todas
as alegrias ou tristezas do mundo, esse texto foi mais ou menos traduzido assim:"isso também passará". É nossa alegria e tristeza anunciar a todos os jogadores que
isso também passou! Esperamos que tenham gostado do seu tempo conosco, tanto quanto gostamos de lhes servir e agradecemos por ficarem até o final!<br /><br />

					Os resultados: o dia há muito passou à noite, mas os trabalhadores em <?php echo "<a href=\"karte.php?d=$vref&c=".$generator->getMapCheck($vref)."\">$winningvillagename</a>"; ?>,
					trabalharam durante a véspera de inverno, todos cautelosos com os incontáveis exércitos marchando para destruir seu trabalho, sabendo que eles correram contra o tempo e a maior
					ameaça que já enfrentaram as pessoas livres. Suas lutas incansáveis foram reafirmadas em <?PhP echo date('d/m/Y', $wingsrtime);?> às <?PhP echo date('H:i:s', $wingsrtime);?> após um
					trabalhador que ficou desconhecido colocar a pedra final no que sempre será conhecida como a maior e mais magnífica criação em toda a história desde a queda dos Natars<br /><br />

					Juntamente com a aliança "<?php echo "<a href=\"allianz.php?aid=$allianceid\">$winningalliancetag</a>"; ?>", "<?php echo "<a href=\"spieler.php?uid=$owner\">$username</a>"; ?>"
					foi o primeiro a terminar a Maravilha do Mundo, usando milhões de recursos, além de protegê-lo com raça dos milhares de defensores valentes. <b><?php echo "<a href=\"spieler.php?uid=$owner\">$username</a>"; ?></b>
					quem recebe o título "Vencedor desta era"!<br /><br />


					"<a href="spieler.php?uid=<?php echo $datas[0]['userid']; ?>" title="Total Villages: <?php echo $datas[0]['totalvillages']; echo "\n";?>Total Population: <?php echo $datas[0]['totalpop']; ?>"><?php echo $datas[0]['username']; ?></a>" foi o governante do maior império pessoal, seguido de perto por "<a href="spieler.php?uid=<?php echo $datas[1]['userid']; ?>" title="Total Villages: <?php echo $datas[1]['totalvillages']; echo "\n";?>Total Population: <?php echo $datas[1]['totalpop']; ?>"><?php echo $datas[1]['username']; ?></a>" e "<a href="spieler.php?uid=<?php echo $datas[2]['userid']; ?>" title="Total Villages: <?php echo $datas[2]['totalvillages']; echo "\n";?>Total Population: <?php echo $datas[2]['totalpop']; ?>"><?php echo $datas[2]['username']; ?></a>".<br />
					"<a href="spieler.php?uid=<?php echo $attacker[0]['userid']; ?>" title="Total Villages: <?php echo $attacker[0]['totalvillages']; echo "\n"; ?>Attack Points: <?php echo $attacker[0]['apall']; ?>"><?php echo $attacker[0]['username']; ?></a>" matou mais do que qualquer outro, e foi o comandante mais poderoso e temível.<br />
					"<a href="spieler.php?uid=<?php echo $defender[0]['userid']; ?>" title="Total Villages: <?php echo $defender[0]['totalvillages']; echo "\n"; ?>Defence Points: <?php echo $defender[0]['dpall'];?>"><?php echo $defender[0]['username']; ?></a>" era o defensor mais glorioso, matando inimigos nos portões da aldeia, manchando as terras ao redor dessas aldeias com sangue.
					<br /><br />
					<p>Parabéns para todos.</p>
					<br /><br />
					Cumprimentos,<br />
					 Time <?php echo SERVER_NAME;?>.<br /><br /><br /><br />
					<small><i>(Por: aggenkeech & Eyas95, tradução e correções Spektrwn)</i></small></p>

					<br /><br />
					<center><a href="dorf1.php">&raquo; Continuar... a ver.</a></center>
				</div>
			...
			<div id="stime">
				<div id="ltime">
					<div id="ltimeWrap">
						Calculado em <b><?php echo round(($generator->pageLoadTimeEnd()-$start)*1000);?></b> ms
						<br/>Horário do servidor: <span id="tp1" class="b"><?php echo date('H:i:s'); ?></span>
					</div>
				</div>
			</div>
		<div id="ce">
