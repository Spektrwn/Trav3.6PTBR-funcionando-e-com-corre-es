			...
			<tr>
				...
				<td>
					<select name="reason" class="fm">
						<?php
							$arr = array('Forçando','Enganação','Hack','Erro no jogo','Nome ruim','Multi contas','Promeças');
							foreach($arr as $r)
							{
				...
				<td>
					<select name="time" class="fm">
						<?php
							...
							foreach($arr2 as $r)
							{
								echo '<option value="'.($r*3600*24).'">'.$r.' dias/s</option>';
							}
							echo '<option value="">Para sempre</option>';
						?>
					</select>
				</td>
			</tr>
			<tr>
				<td colspan="2" class="on"><input type="image" src="../img/admin/b/ok1.gif" value="submit"></br>Aviso: se o jogador tem 'em nome, você precisa escrever sua identificação para bani-lo.</td>
			</tr>
		</tbody>
	</table>
</form>
...
<table id="member" cellpadding="1" cellspacing="1">
	<thead>
		<tr>
			<th colspan="6">Jogadores Banidos (<?php echo count($bannedUsers); ?>)</th>
		</tr>
		<tr>
			<td><b>Nome do usuário</b></td>
			<td><b>Duração (de / para)</b></td>
			<td><b>Razão</b></td>
			<td></td>
		</tr>
		</thead>
			...
			if($bannedUsers)
			{
				for ($i = 0; $i <= count($bannedUsers)-1; $i++)
				{
					...
					<tr>
						...
						<td class="on"><a href="?action=delBan&uid='.$bannedUsers[$i]['uid'].'&id='.$bannedUsers[$i]['id'].'" onClick="return del(\'unban\',\''.$name.'\')"><img src="../img/Admin/del.gif" class="del" title="cancelar" alt="cancelar"></img></a></td>
					</tr>';
				}
			}
			else
			{
				echo '<tr><td colspan="6" class="on">Nenhum jogador está banido</td></tr>';
			}
		?>
	</tbody>
</table>