...
class Units {
    ...
    private function loadUnits($post) {
        global $database,$village,$session,$generator,$logging,$form;
                ...
                $form->addError("error","Você não pode reforçar esta aldeia / oásis");
                }
                if($disabled != "" && $post['c'] == 3){
                $form->addError("error","Você não pode atacar esta aldeia / oásis com ataque normal");
                }
                if(    !$post['t1'] && !$post['t2'] &&  !$post['t3'] && !$post['t4'] && !$post['t5'] &&
                    !$post['t6'] && !$post['t7'] &&  !$post['t8'] && !$post['t9'] && !$post['t10'] &&  !$post['t11']){
                $form->addError("error","Você precisa marcar no mínimo uma tropa");
                }

                if(!$post['dname'] && !$post['x'] && !$post['y']){
                $form->addError("error","Inserir nome ou coordenadas");
                }

                if(isset($post['dname']) && $post['dname'] != "") {
                    $id = $database->getVillageByName(stripslashes($post['dname']));
                    if (!isset($id)){
                    $form->addError("error","Aldeia não existe");
                    }else{
                ...
                if(isset($post['x']) && isset($post['y']) && $post['x'] != "" && $post['y'] != "") {
                    ...
                    if (!$database->getVillageState($id)){
                        $form->addError("error","As coordenadas não existem");
                    }
                    ...
                    for($i=1; $i<11; $i++)
                    {
                        if(isset($post['t'.$i]))
                        {
						 //Retirado Spektrwn devido falha ao enviar os colonos
                ...
                if($database->hasBeginnerProtection($id)==1) {
                    $form->addError("error","Jogador está sob a proteção de iniciantes. Você não pode atacá-lo");
                }
                ...
                $userAccess = $database->getUserField($villageOwner,'access',0);
                    if($userAccess == '0' or $userAccess == '8' or $userAccess == '9'){
                                $form->addError("error","O Jogador está Banido. Você não pode atacá-lo");
                                //break;
                    }

                //check if attacking same village that units are in
                    if($id == $village->wid){
                                $form->addError("error","Você não pode atacar a mesma aldeia de que você está enviando.");
                                //break;
                    }
...
if($session->access != BANNED){
        ...
        if(($enforce['from']==$village->wid) || ($enforce['vref']==$village->wid)){
            ...
            $form->addError("error","Você não pode mudar tropas de alguém.");
                if($form->returnErrors() > 0) {
                    $_SESSION['errorarray'] = $form->getErrors();
                    $_SESSION['valuearray'] = $_POST;
                    header("Location: a2b.php");
