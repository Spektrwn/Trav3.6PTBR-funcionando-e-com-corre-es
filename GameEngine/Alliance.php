		...
		*****************************************/
		public function sendInvite($post) {
			...
			if($this->userPermArray['opt4'] == 0) {
				...
				// Log the notice
				$database->insertAlliNotice($session->alliance, '<a href="spieler.php?uid=' . $session->uid . '">' . addslashes($session->username) . '</a> convidou  <a href="spieler.php?uid=' . $UserData['id'] . '">' . $UserData['username'] . '</a> para a aliança.');
			}
			...
			if($this->userPermArray['opt4'] == 0) {
				...
				$database->sendInvitation($UserData['id'], $aid, $session->uid);
				// Log the notice
				$database->insertAlliNotice($session->alliance, '<a href="spieler.php?uid=' . $session->uid . '">' . addslashes($session->username) . '</a> convidou  <a href="spieler.php?uid=' . $UserData['id'] . '">' . addslashes($UserData['username']) . '</a> para a aliança.');
			}
			}
			}else{
		...
		private function rejectInvite($get) {
			...
			foreach($this->inviteArray as $invite) {
				if($invite['id'] == $get['d']) {
					...
					$database->insertAlliNotice($invite['alliance'], '<a href="spieler.php?uid=' . $session->uid . '">' . addslashes($session->username) . '</a> rejeitou o convite.');
				}
			}
		...
		private function delInvite($get) {
			...
			foreach($inviteArray as $invite) {
				...
				$invitename = $database->getUserArray($invite['uid'], 1);
					$database->removeInvitation($get['d']);
					$database->insertAlliNotice($session->alliance, '<a href="spieler.php?uid=' . $session->uid . '">' . addslashes($session->username) . '</a> retirou o convite de <a href="spieler.php?uid=' . $invitename['id'] . '">' . $invitename['username'] . '</a>.');
				}
			}
		...
		private function acceptInvite($get) {
			...
			if($session->alliance == 0){
				...
				if(count($memberlist) < $alliance_info['max']){
					...
					// Log the notice
					$database->insertAlliNotice($invite['alliance'], '<a href="spieler.php?uid=' . $session->uid . '">' . addslashes($session->username) . '</a> se juntou à aliança.');
				}else{
				$accept_error = 1;
			...
			if($accept_error == 1){
			$form->addError("ally_accept", "A aliança pode conter apenas ".$max." pessoas agora.");
			}else{
			header("Location: build.php?id=" . $get['id']);
			}
			}else{
		...
		*****************************************/
		private function createAlliance($post) {
			...
			if($form->returnErrors() != 0) {
				...
				// Asign Permissions
				$database->createAlliPermissions($database->RemoveXSS($session->uid), $database->RemoveXSS($aid), 'Fundador da Aliança', '1', '1', '1', '1', '1', '1', '1', '1');
				// log the notice
				$database->insertAlliNotice($aid, 'A aliança foi fundada por <a href="spieler.php?uid=' . $session->uid . '">' . addslashes($session->username) . '</a>.');
				header("Location: build.php?id=" . $post['id']);
			}
			}else{
			header("Location: banned.php");
		...
		*****************************************/
		private function changeAliName($get) {
			...
			if($form->returnErrors() != 0) {
				...
				// log the notice
				$database->insertAlliNotice($session->alliance, '<a href="spieler.php?uid=' . $session->uid . '">' . addslashes($session->username) . '</a> mudou o nome da aliança.');
			}
			}else{
			header("Location: banned.php");
			}
		...
		*****************************************/
		private function updateAlliProfile($post) {
			...
			if($form->returnErrors() != 0) {
				...
				// log the notice
				$database->insertAlliNotice($session->alliance, '<a href="spieler.php?uid=' . $session->uid . '">' . addslashes($session->username) . '</a> Mudou a descrição da aliança.');
			}
			}else{
			header("Location: banned.php");
			}
		...
		*****************************************/
		private function changeUserPermissions($post) {
			...
			if($form->returnErrors() != 0) {
				...
				// log the notice
				$database->insertAlliNotice($session->alliance, '<a href="spieler.php?uid=' . $session->uid . '">' . addslashes($session->username) . '</a> alterou permissões.');
			}
			}else{
			header("Location: banned.php");
			}
		...
		*****************************************/
		private function kickAlliUser($post) {
			...
			if($this->userPermArray['opt2'] == 0) {
				...
				// log the notice
				$database->insertAlliNotice($session->alliance, '<a href="spieler.php?uid=' . $UserData['id'] . '">' . addslashes($post['a_user']) . '</a> saiu da aliança.');
		...
		*****************************************/
		private function quitally($post) {
			...
			if(!isset($post['pw']) || $post['pw'] == "") {
				...
				// log the notice
				$database->deleteAlliance($session->alliance);
				$database->insertAlliNotice($session->alliance, '<a href="spieler.php?uid=' . $session->uid . '">' . addslashes($session->username) . '</a> saiu da aliança.');
				...
				if($database->getAllianceID($aName) != $session->alliance) {
					if($aType >= 1 and $aType <= 3) {
						if(!$database->diplomacyInviteCheck($database->getAllianceID($aName), $session->alliance)) {
							...
							$form->addError("name", "Convidar enviado");
						} else {
							$form->addError("name", "Você já enviou um convite para eles");
						}

					} else {
						$form->addError("name", "escolha feita errada");
					}
				} else {
					$form->addError("name", "Você não pode convidar sua própria aliança");
				}
			} else {
				$form->addError("name", "Aliança não existe");
			}
			}else{
			header("Location: banned.php");
			}
