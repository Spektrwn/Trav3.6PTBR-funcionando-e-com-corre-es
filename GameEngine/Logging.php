...
class Logging {
	...
	public function addBuildLog($wid,$building,$level,$type) {
		...
		if(LOG_BUILD) {
			if($type) {
				$log = "Iniciar a construção de ";
			}
			else {
				$log = "Iniciar atualização de ";
			}
			$log .= $building." ao nível ".$level;
	...
	public function addTechLog($wid,$tech,$level) {
		global $database;
		if(LOG_TECH) {
			$log = "Atualização de tecnologia ".$tech." ao nível ".$level;
	...
	public function goldFinLog($wid) {
		global $database;
		if(LOG_GOLD_FIN) {
			$log = "Concluir a construção e pesquisa com ouro";
	...
	public function addMarketLog($wid,$type,$data) {
		...
		if(LOG_MARKET) {
			if($type == 1) {
				$log = "Enviei ".$data[0].",".$data[1].",".$data[2].",".$data[3]." à aldeia ".$data[4];
			}
			else if($type == 2) {
				$log = "Recurso trocado entre ".$wid." e ".$data[0]." referência de mercado é ".$data[1];
			}
