...
class adm_DB {
  ...
  function adm_DB(){
	...
	function Login($username,$password){
		...
		if($dbarray['password'] == md5($password)) {
			mysql_query("Insert into ".TB_PREFIX."admin_log values (0,'X','$username Acessado em (IP: <b>".$_SERVER['REMOTE_ADDR']."</b>)',".time().")");
			return true;
		}
		else {
			mysql_query("Insert into ".TB_PREFIX."admin_log values (0,'X','<font color=\'red\'><b>IP: ".$_SERVER['REMOTE_ADDR']." Tentou acessar com nome de usuário <u> $username</u> Mas o acesso foi negado!</font></b>',".time().")");
			return false;
		}
	}
  ...
  function buildingPOP($f,$lvl){
	...
	$status = 0;
	if($status == 0){
	mysql_query("Insert into ".TB_PREFIX."admin_log values (0,".$_SESSION['id'].",'Nova aldeia adicionada <b><a href=\'admin.php?p=village&did=$wid\'>$wid</a></b> ao usuário <b><a href=\'admin.php?p=player&uid=$uid\'>$uid</a></b>',".time().")");
	 ...
	 global $database;
	   ...
	   $user = $database->getUserArray($post['uid'],1);
		  for ($i = 0; $i <= count($villages)-1; $i++) {
			...
			mysql_query("Insert into ".TB_PREFIX."admin_log values (0,".$_SESSION['id'].",'Usuário punido: <a href=\'admin.php?p=player&uid=".$post['uid']."\'>".$post['uid']."</a> com <b>-".$post['punish']."%</b> população',".time().")");
  }
  ...
  function DelUnits2($vid,$unit){
	...
	$ID = $_SESSION['id'];//$database->getUserField($_SESSION['username'],'id',1);
	   if($this->CheckPass($pass,$ID)){
		...
		mysql_query("Insert into ".TB_PREFIX."admin_log values (0,$ID,'Usuário deletado <a>$name</a>',".time().")");
  ...
  function CheckPass($password,$uid){
	...
	mysql_query("Insert into ".TB_PREFIX."admin_log values (0,".$_SESSION['id'].",'Aldeia apagada <b>$wref</b>',".time().")");
	...
	mysql_query("Insert into ".TB_PREFIX."admin_log values (0,".$_SESSION['id'].",'Usuário desbanido <a href=\'admin.php?p=player&uid=$uid\'>$name</a>',".time().")");
  ...
  function AddBan($uid,$end,$reason){
	global $database;
	$name = addslashes($database->getUserField($uid,"username",0));
	mysql_query("Insert into ".TB_PREFIX."admin_log values (0,".$_SESSION['id'].",'Usuário banido <a href=\'admin.php?p=player&uid=$uid\'>$name</a>',".time().")");
