...
class MYSQL_DB {
	...
	*****************************************/
	...
	public function conquerOasis($vref,$wref) {
		...
		$uid = $vinfo['owner'];
		$q = "UPDATE `".TB_PREFIX."odata` SET conqured=$vref,loyalty=100,lastupdated=".time().",owner=$uid,name='Oasis ocupado' WHERE wref=$wref";
		return mysql_query($q, $this->connection);
	}
	...
	function removeOases($wref) {
		$q = "UPDATE ".TB_PREFIX."odata SET conqured = 0, owner = 2, name = 'Oasis desocupado' WHERE wref = $wref";
		return mysql_query($q, $this->connection);
	}
	...
	***************************/
	...
	function populateOasisdata() {
		...
		while($row = mysql_fetch_array($result2)) {
			...
			$q = "INSERT into " . TB_PREFIX . "odata VALUES ('" . $basearray['id'] . "'," . $basearray['oasistype'] . ",0,800,800,800,800,800,800," . time() . ",100,2,'Oasis desocupado',".rand(0,2).")";
			$result = mysql_query($q, $this->connection);
		}
	}
